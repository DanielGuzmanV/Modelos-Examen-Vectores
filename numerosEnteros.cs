...
using System.Threading.Tasks;
    ...
    class numerosEnteros
    {
        ...
        // verificar si es capicua 3
        public bool verifCapicua()
        {
            numerosEnteros newNumber = new numerosEnteros();
            bool respuesta = false;

            newNumber.cargarDatos(this.numeros);
            newNumber.invertirNum();

            if (this.numeros == newNumber.numeros)
            {
                respuesta = true;
            }
            return respuesta;
        }
        // Verificar si es capicua 2 
        public bool verifCapicua2(int number)
        {
            ...
            {
                digito = dato % 10;
                numResult = (numResult * 10) + digito;
                dato = dato / 10;

            }
            if (numOrigin == numResult)
            {
                respuesta = true;
            }
            number = dato;
